...
class Util {
  ...
  getToken (signer) {
    let ABI = require(path.resolve(__dirname, "../abi/contracts/C0.sol/C0.json"))
  ...
  async clone (address, index, name, symbol, val) {
    let FACTORYABI = require(path.resolve(__dirname, "../abi/contracts/Factory.sol/Factory.json"))
    let _interface = new ethers.utils.Interface(FACTORYABI)
    if (val) {
      let tx = await this.factory.genesis(index, name, symbol, val)
      await tx.wait()
    } else {
      let tx = await this.factory.genesis(index, name, symbol)
      let r = await tx.wait()
    }

    let addr = await this.factory.find(address, 0, 1)
